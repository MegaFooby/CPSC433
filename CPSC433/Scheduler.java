...
public class Scheduler {
    ...
    public static double TOTAL_FACTS = 0;
    ...
    public static void main(String args[]) {
        ...
        Option psc = new Option("psc", "pensec", true, "Section penalty");
        options.addOption(psc);
        ...
        if(cmd.hasOption("psc"))
			pen_secdiff = Integer.parseInt(cmd.getOptionValue("psc"));
		...
		TOTAL_FACTS = Math.pow(parse.slots.size(), parse.slots.size());
        ...
        System.out.println(solution.slots[0].time + " " + solution.slots[0].coursemax + " " + solution.slots[0].course.size());
		if(solution.score < -1000){
			System.out.println("No valid solutions found with score: " + Integer.toString(solution.score));
			System.exit(0);
		} 
    ...
    public static Fact and_tree(Fact current, Parser parse) {
        ...
        for(int i = 1; i < possible.size(); i++) {
			//System.out.println(possible.get(i).score);
        	if(solution.score < possible.get(i).score) {
        		solution = possible.get(i);
        	}
        }
        return solution;
...
class Course {
    ...
    public boolean equals(Course compare) {
    	if (this.is_lecture == compare.is_lecture && this.is_lecture == false) {
    		return (this.name.equals(compare.name) && this.number == compare.number &&
                    ...
                    this.tutorial_num == compare.tutorial_num);
    	}
        return (this.name.equals(compare.name) && this.number == compare.number &&
                this.lecture_num == compare.lecture_num && this.is_lecture == compare.is_lecture);
    }
...
class Fact {
    ...
    public boolean constr(int slotnum, int coursenum, Parser parse){
        ...
        if (unassigned.size() == 0) {
			for(int i = 0; i < this.slots.length; i++) {
				int lecnum = 0, tutnum = 0;
				for(int j = 0; j < this.slots[i].course.size(); j++) {
					if(this.slots[i].course.get(j).is_lecture) {
						lecnum++;
					} else {
						tutnum++;
					}
				}
				if(lecnum < this.slots[i].coursemin) {
					return false;
				}
				if(tutnum < this.slots[i].labmin) {
					return false;
				}
			}
        	return true;
        ...
        /*if(this.unassigned.get(coursenum).is_lecture) {
			if(this.slots[slotnum].coursemax < this.slots[slotnum].course.size()+1){
				return false;
			}
		}
		if(!this.unassigned.get(coursenum).is_lecture) {
			if(this.slots[slotnum].labmax < this.slots[slotnum].asslab) {
				return false;
			}
		}*/
    ...
    public void evaluate(int slot, Parser parse) {
        ...
        int score = 0;
        if(!this.constr(slot, this.unassigned.size()-1, parse)) {
        	this.score = -100000000;
        	this.slots[slot].course.add(this.unassigned.get(this.unassigned.size()-1));
            this.unassigned.remove(this.unassigned.get(this.unassigned.size()-1));
        }
        else {
        	this.slots[slot].course.add(this.unassigned.get(this.unassigned.size()-1));
            this.unassigned.remove(this.unassigned.get(this.unassigned.size()-1));
